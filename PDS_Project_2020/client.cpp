...
*/
int Client::send(int operation, std::string folderPath, std::string fileName, std::string content, std::uintmax_t file_size,
    ...
    if (operation == 1 || operation == 3) {
        ...
        myLogger->info("[OPERATION_" + std::to_string(operation) + "]: SEND CONF STREAM RESP returned code: " + std::to_string(resCode));
        myLogger->flush();
        ...
        myLogger->info("[OPERATION_" + std::to_string(operation) + "]: RCV END STREAM returned code: " + std::to_string(resCode));
        myLogger->flush();
...
*/
int Client::sendMessage(message2 msg, std::atomic<bool>& b) {
...
*/
int Client::sendFileStream(std::string filePath, std::atomic<bool>& b) {
...
*/
int Client::readMessageResponse(std::string& response) {
...
*/
int Client::readInitialConfStream(int packetsNumber, std::string conf) {
    ...
    try {
        ...
        do {
            ...
            std::string tempBuf = (char*)buffer;
            conf += tempBuf;
